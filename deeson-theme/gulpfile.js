...
gulp.task('style', getTask('style', {
  src: './src/scss/*.scss',
  dest: './deeson-theme/css',
  imagePath: '../img/',
  includePaths: ['normalize.css']
}));
gulp.task('move-fonts', getLmnTask('copy', {
  src: './src/fonts/**.*',
  dest: './deeson-theme/fonts/'
}));
gulp.task('move-mkdocs-templates', getLmnTask('copy', {
  src: './**.html',
  dest: './deeson-theme/'
}));


gulp.task('default', ['html', 'js', 'style', 'images', 'move-fonts','move-mkdocs-templates'], getTask('default'));
